...
List_Vektor<T> List_Vektor<T>::concat(List_Vektor<T> &l){
    List_Vektor<T> temp;
    temp.array_dimension_=l.array_dimension_+this->array_dimension_;
    temp.length_=l.length_+this->length_;
    // delete elements_;
    temp.elements_=new T[temp.array_dimension_];
    for (int i = 0; i < temp.length_; ++i)
    {
        temp.elements_[i]=elements_[i];
    }
    int j=0;
    for (int i = length_; i < temp.length_; ++i)
    {
